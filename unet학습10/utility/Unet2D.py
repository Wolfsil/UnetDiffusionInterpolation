...
def UnetModel(inputShape=(128, 128, 4)):
    ...
    outputImage = tf.keras.layers.SeparableConv2D(4, kernel_size=3, padding="same")(
        tf.keras.layers.Concatenate()([primaryImage, d1, secondaryImage])
    )
    outputImage = tf.keras.layers.SeparableConv2D(4, kernel_size=1, padding="same")(
        tf.keras.layers.Concatenate()([primaryImage, outputImage, secondaryImage])
    )
