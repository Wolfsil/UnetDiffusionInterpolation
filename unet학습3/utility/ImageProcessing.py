...
def LoadGifExtract(path, extractFrame=5, paddingSize=32):
    gif = Image.open(path)
...
def DatasetGenerater(gifPath):
    # gif파일을 반환
    for i in gifPath:
        inputImage, outputImage = DivideConcatenate(LoadGifExtract(i))  # 12채널, 8채널
        ...
        noisyImage = sigRate * outputImage + noiseRate * noise  # 8 채널
...
def SaveGif(path, inputImage, outputImage):
    ...
    imgs.append(Image.fromarray(outputImage[1].round().astype(np.int8), mode="RGBA"))

    imgs.append(Image.fromarray(inputImage[2].round().astype(np.int8), mode="RGBA"))
